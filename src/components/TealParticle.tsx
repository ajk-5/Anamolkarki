...
export default function TealParticles({ particleCount = 50 }: TealParticlesProps) {
  ...
  const randomViewportWidth = () => `${Math.random() * 100}vw`;
  const randomViewportHeight = () => `${Math.random() * 100}vh`;

  useEffect(() => {
    const positions = Array.from({ length: particleCount }, () => ({
      x: randomViewportWidth(),
      y: randomViewportHeight(),
    }));
    ...
    <div className="absolute inset-0 pointer-events-none w-full h-full overflow-hidden z-0">
      {particlePositions.map((pos, i) => (
        <MotionDiv
          ...
          animate={{
            x: [pos.x, randomViewportWidth()],
            y: [pos.y, randomViewportHeight()],
            scale: [0, 1, 0],
            opacity: [0, 0.8, 0],
            transition: {
