...
export default function Home() {
  ...
  useEffect(() => {
    const onWheel = (event: WheelEvent) => {
      ...
      const nestedCarousel = target?.closest<HTMLElement>("[data-nested-carousel]");
      if (nestedCarousel) {
        const edge = nestedCarousel.getAttribute("data-edge") ?? "middle";
        ...
        if (
          !(
            verticalDominant &&
            ((edge === "start" && scrollingUp) || (edge === "end" && scrollingDown))
          )
        ) {
          return;
        }
  ...
  return (
    ...
    >
      <div className="relative flex-1 min-h-0">
        <div className="h-full flex items-center justify-center px-1 sm:px-4">
          <div
          ...
          >
            <button
              ...
              className="btn-icon absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-30 hidden h-10 w-10 p-0 sm:inline-flex sm:h-12 sm:w-12"
            >
              <svg
            ...
            <button
              ...
              className="btn-icon absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-30 hidden h-10 w-10 p-0 sm:inline-flex sm:h-12 sm:w-12"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="22"
